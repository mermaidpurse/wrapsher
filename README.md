...
programming language, you need to deal with distribution, dependency
managament and installation concerns. Here are some places where
shell scripts still exist, and in numbers:

- Installers (e.g. [rustup](https://github.com/rust-lang/rustup/blob/master/rustup-init.sh))
- System initialization (e.g. user-data scripts)
...
imagine--with work--how some of them are addressable using no shell at
...
you want to _run_ a shell script, but you still don't want to _write_ it.
That's the problem Wrapsher is designed to solve. You write your program
in an attractive, safe language with the affordances you need to write
good code, but you run it using only shell.
...
rich expressions) and execute it in a "ubiquitous" runtime. There's
