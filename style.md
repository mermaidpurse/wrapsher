...
- When making reference to the following in prose (not literal
  code examples):
    - Typeset module names in **bold**, e.g., "the **io** module".
    - Typeset other literal identifiers using backticks
      (<code>`</code>), e.g., "as an example of using
      the `use version` constraint...".
    - Typeset variable values and other things that you're
      referring to metasyntactically--that is, where the user
      must provide a value but you want to refer to whatever
      that value is in prose, in italics, e.g. "this allows
      the **io** module to accept _username_ as input".
