...
module Wrapsher
  class Syntax < Parslet::Parser
    ...
    rule(:expression)               { (assignment | postfix_chain | conditional | fun_call | shellcode_call | boolean_not | secondary_op | primary_op | boolean_op | comparison | term) }
    ...
    rule(:boolean_not)              { ((str('not') >> space | str('!') >> space?) >> expression.as(:subject)).as(:boolean_not) }
