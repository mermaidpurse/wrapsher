...
_wsh_rv=$?
_wsh_line="postamble.sh:7"
_wsh_check_return "${_wsh_rv}" "(top) at ${_wsh_line}" || _wsh_panic $? "list.new()"
...
do
  ...
  _wsh_rv=$?
  _wsh_line="postamble.sh:15"
  _wsh_check_return "${_wsh_rv}" "(top) at ${_wsh_line}" || _wsh_panic $? "list.push(${_wshv_argv}, ${_wshi})"
  _wshv_argv="${_wsh_result}"
done
...
_wsh_rv=$?
_wsh_line="postamble.sh:21"
_wsh_check_return "${_wsh_rv}" "(top) at ${_wsh_line}" || _wsh_panic $? "main(${_wshv_argv})"
