...
<i>newtype</i> <i>othertype</i></code>, you're essentially saying that
you are going to use _othertype_ to store sufficient information about
...
- Using errors for flow control, i.e. try/catch to deal with errors
  ...
  things cleaner and safer before stopping; not to continue down n
  alternate path.
- Use of complex return values that have to be checked for
  ...
  "next to" a function return value.  Similarly, a strong convention
  (like Go's) of returning more than one value, one of which is an
  error flag you always need to check, isn't really encouraged[^1]
